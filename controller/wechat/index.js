...
export default class {
    //微信与服务器的验证
    static async get_wechat(ctx, next) {
    ...
    //处理微信推送的消息
    static async post_wechat(ctx, next) {
    ...
    //处理微信用户验证
    static async userinfo_wechat(ctx, next) {
    ...
    static async start(ctx, next) {
        ctx.redirect(`https://open.weixin.qq.com/connect/oauth2/authorize?appid=${config.appID}&redirect_uri=${encodeURIComponent(config.urlPrefix + '/api/authorization')}&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect`)
    }
