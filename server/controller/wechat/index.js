...
export default class {
  //微信与服务器的验证
  static async get_wechat(req, res) {
  ...
  //处理微信推送的消息
  static async post_wechat(req, res) {
    ...
    if (message.msgtype === 'event') {
      ...
      if (message.event === 'subscribe') {
        ...
        if (message.eventkey) {
          //用户扫码关注公众号
          //后续需要对参数进行判断
          const username = message.eventkey.slice(message.eventkey.indexOf(' ') + 1)
  ...
  static async start(req, res) {
    let username = req.query.username || 'Por'
