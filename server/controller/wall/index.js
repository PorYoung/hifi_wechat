...
export default class {
  ...
  static async setUI(req, res) {
    let info = req.body
    if (!info || !info.username) return res.send('-1')
    if (info.hasOwnProperty('background')) {
      await db.wall.findOneAndUpdate({ username: info.username }, { $set: { 'UI.background': info.background } })
    }
    if (info.hasOwnProperty('avatar')) {
      await db.wall.findOneAndUpdate({ username: info.username }, { $set: { 'UI.avatar': info.avatar } })
    }
    if (info.hasOwnProperty('bgtext')) {
      await db.wall.findOneAndUpdate({ username: info.username }, { $set: { 'UI.bgtext': info.bgtext } })
    }
    if (info.hasOwnProperty('bgtextContent')) {
      await db.wall.findOneAndUpdate({ username: info.username }, { $set: { 'UI.bgtextContent': info.bgtextContent } })
    }
    return res.send('1')
  }
  ...
  static async saveGuests(req, res) {
    let info = req.body
  ...
  static async uploadOptionsImage(req, res) {
  ...
  static async switchVote(req, res) {
    let vote = req.body
  ...
  static async getActiveVote(req, res) {
    let username = req.query.username
